    ...
    <div aria-hidden="true" className={cn("flex h-full w-full flex-col gap-3", className)}>
      ...
      <div className="flex flex-1 items-end gap-1.5 border-b border-border pb-px sm:gap-2">
        {BAR_HEIGHTS.map((height, i) => (
          <Skeleton
            key={i}
            className="min-w-0 flex-1 rounded-b-none bg-slate-200"
      ...
      <div className="flex items-center justify-between gap-2">
        {Array.from({ length: 6 }).map((_, i) => (
          <Skeleton key={i} className="h-2 w-8 bg-slate-100" />
        ))}
      </div>
    </div>
  );
    ...
    <div aria-hidden="true" className="border-t border-border pt-2">
      ...
      <ul className="space-y-3 md:hidden">
        {Array.from({ length: rows }).map((_, i) => (
          <li key={i} className="rounded-xl border border-border p-4">
            ...
            <Skeleton className="mt-3 h-4 w-2/3 bg-slate-200" />
            <Skeleton className="mt-2 h-3 w-1/3 bg-slate-100" />
            <Skeleton className="mt-4 h-4 w-1/2 bg-slate-200" />
            <Skeleton className="mt-2 h-3 w-5/6 bg-slate-100" />
          </li>
        ))}
      </ul>
      ...
      <div className="hidden md:block">
        <div className="flex gap-4 border-b border-border py-3">
          ...
          <Skeleton className="h-3 flex-1 bg-slate-100" />
